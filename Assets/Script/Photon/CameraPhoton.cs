...
public class CameraPhoton : MonoBehaviourPun
{
    ...
    void Start()
    {
        ...
        CameraPhoton _cameraPhoton = this.gameObject.GetComponent<CameraPhoton>();

        // Kiem tra instance va client
        if (_cameraPhoton != null){
            if (photonView.IsMine){
                // Cho camera theo client tuong ung
                _cameraPhoton.OnStartFollowing();
            }
        }
        else {
            Debug.LogError("<Color=Red><a>Missing</a></Color> CameraPhoton component on playerPrefab", this);
        }
    }
